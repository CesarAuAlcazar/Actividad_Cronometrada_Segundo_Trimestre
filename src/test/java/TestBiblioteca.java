...
class TestBiblioteca {
    ...
    void encuentraLibroPorTitulo() {
        ...
        Libro librook = biblioteca.encuentraLibroPorTitulo("Ruslan y Ludmila"); // Buscamos un libro concreto
        assertNotNull(librook); // Verificamos que se ha encontrado el libro
        assertEquals(this.libro, librook); //Nos aseguramos que es el libro que buscamos

        Libro libroNo = biblioteca.encuentraLibroPorTitulo("Este libro no está en la biblioteca");
        assertNull(libroNo); // Verifica que el libro no se ha encontrado
    }
    ...
    void encuentraLibrosPorAutor() {
        ...
        List<Libro> libro1 = biblioteca.encuentraLibrosPorAutor("Alexander Pushkin"); // Buscamos un autor
